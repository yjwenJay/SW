...
{
    public class AdminPageBlockManage : PageBase
    {
        ...
        void edit_column_visibility()
        {
            ...
            {
                json.successS = true;
                json.SingleInfo = Microsoft.JScript.GlobalObject.escape(Html.HrefLink("Core", "AdminPageBlockManage", "pid=" + page.PageId));
            }
            else
            {
                json.successS = false;
        ...
        protected string BlockItemHtml(PageBlockList blist, List<BlockInfo> lst, string btype)
        {
            ...
            foreach (PageBlock item in blist)
            {
                ...
                sb.Append("         &raquo;<ul>");
                sb.Append("             <li><a href=\"" + Html.HrefLink("Core", "AdminPageBlockEdit", "pid=" + page.PageId, "btype=" + btype, "pbid=" + item.PageBlockId) + "\">编辑属性</a></li>");
