...
{
    public class SHA256RSA2Helper
    {
        public static string Encrypt(string content, Encoding encode, string privateKeyPem)
        {
            RSACryptoServiceProvider rsaCsp = LoadCertificateString(privateKeyPem, "RSA2");
            byte[] dataBytes = null;
            dataBytes = encode.GetBytes(content);
            byte[] signatureBytes = rsaCsp.SignData(dataBytes, "SHA256");
            return Convert.ToBase64String(signatureBytes);
        }

        public static bool CheckSign(string publicKeyPem, string signContent, string sign, Encoding encode)
        {
            ...
            bool bVerifyResultOriginal = rsa.VerifyData(encode.GetBytes(signContent), "SHA256", Convert.FromBase64String(sign));
